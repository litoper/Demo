...
public class LoginNormalPresenter extends BaseFragmentPresenter<LoginFragContract.View> implements LoginFragContract.Presenter<LoginFragContract.View> {
    ...
    public void login(String username, String password) {
        if (NullUtils.isEmpty(username) || NullUtils.isEmpty(password)) {
            // TODO: 2018/6/1  
            Toast.makeText(null, "密码或者账户不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
        Observer<BaseResponse<List<LoginBean>>> progress = new SubProgress<>(null, SubDialog.NODIALOG, new SubNextImpl<BaseResponse<List<LoginBean>>>() {
            @Override
